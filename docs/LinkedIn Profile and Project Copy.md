# LinkedIn profile and project copy

## Featured item title

**LTECwithLance product ecosystem: applied AI, cybersecurity, and learning technology portfolio**

## Featured item description

A portfolio of interactive apps, editable toolkits, and practical training resources designed to make human judgment teachable across responsible AI, AI governance, cybersecurity, experiential learning, narrative strategy, and controlled venture research.

## Umbrella project entry

**Project name:** LTECwithLance product ecosystem

**Associated with:** LTECwithLance

**Project URL:** https://github.com/TheWhiz13/LTECwithLance-Portfolio

**Description:** Designed and developed an integrated portfolio of decision-support apps, cybersecurity learning tools, experiential learning products, and implementation toolkits. The ecosystem pairs lightweight web apps with editable resources, training pathways, and evidence-centered workflows so users can move from understanding a problem to acting on it responsibly. Product areas include responsible AI, AI governance, cybersecurity situational awareness, scenario practice, financial decision-making, narrative strategy, and controlled venture research.

## Individual project entries

### Should I AI This?

**URL:** https://wisecheck.ltecwithlance.com/

**Source repository:** Private

**Short description:** AI-use suitability and responsible-use guidance before a person pastes, prompts, or uploads.

**Portfolio framing:** Converts responsible AI from a vague policy concern into a concrete pre-use decision habit.

**Toolkit connection:** Responsible AI Toolkit for College Faculty

**Toolkit purchase:** https://ltecwithlance.gumroad.com/l/responsible-ai-toolkit

### AI Governance Check

**URL:** https://governance.ltecwithlance.com/

**Source repository:** Private

**Short description:** Risk routing and governance readiness for proposed AI uses.

**Portfolio framing:** Helps organizations apply more governance only when the risk, data, authority, or consequences justify it.

**Toolkit connection:** Practical AI Governance Toolkit

**Toolkit purchase:** https://ltecwithlance.gumroad.com/l/practical-ai-governance-toolkit

### Cybersecurity Situational Awareness

**URL:** https://cyber.ltecwithlance.com/

**Source repository:** Private

**Short description:** Structured cybersecurity learning, assessment, certificates, and administrator visibility.

**Portfolio framing:** Builds cyber judgment through repeated evidence review and decision practice rather than passive exposure alone.

**Toolkit connection:** Cybersecurity Situational Awareness Toolkit v2.0

**Toolkit purchase:** https://ltecwithlance.gumroad.com/l/cybersecurity-toolkit

### Cybersense: Ready Now

**URL:** https://detect.ltecwithlance.com/

**Source repository:** Private

**Short description:** Short cybersecurity scenario practice, tactic recognition, readiness feedback, and recovery guidance.

**Portfolio framing:** Reinforces formal cyber training with quick, practical decision repetitions between structured sessions.

**Toolkit connection:** Cybersecurity Situational Awareness Toolkit v2.0 companion

**Toolkit purchase:** https://ltecwithlance.gumroad.com/l/cybersecurity-toolkit

### Mission 20

**URL:** https://mission20.ltecwithlance.com/

**Source repository:** Private

**Short description:** Creative $20 constraint challenges for decision-making, value reasoning, resourcefulness, and reflection.

**Portfolio framing:** Turns playful challenges into evidence of resource allocation, opportunity cost, value assessment, constrained decision-making, and financial agency.

**Toolkit connection:** Mission 20 Financial Decision-Making Toolkit

### The Last Archive

**URL:** https://archive.ltecwithlance.com/

**Source repository:** Private

**Short description:** Interactive narrative strategy experience about scarcity, values, and what gets preserved.

**Portfolio framing:** Uses story, scarcity, irreversible choice, and reflection to make values and priorities visible.

**Toolkit connection:** No packaged toolkit today

### LTEC Venture Scientist

**URL:** https://github.com/TheWhiz13/LTECwithLance-Portfolio

**Source repository:** Private

**Short description:** Controlled opportunity research, falsification, evidence review, and proposed experiments for human approval.

**Portfolio framing:** Creates a disciplined research layer for venture exploration without autonomous spending, outreach, trading, bidding, or external commitment.

**Toolkit connection:** Internal operating framework, not a classroom toolkit

## Suggested LinkedIn headline

**Learning Technologist | AI Product Builder | Instructional Designer | Cybersecurity Practitioner**

## Portfolio launch post

I have been building LTECwithLance as a product ecosystem, not just a collection of apps.

The model is simple: **app for engagement + toolkit for implementation + training for adoption + evidence for improvement.**

The current portfolio includes **Should I AI This?**, **AI Governance Check**, **Cybersecurity Situational Awareness**, **Cybersense: Ready Now**, **Mission 20**, **The Last Archive**, and **LTEC Venture Scientist**.

The common thread is human judgment. Each product gives people a concrete decision to make, then supports reflection, documentation, practice, or improvement.

Portfolio: https://github.com/TheWhiz13/LTECwithLance-Portfolio

Toolkit store: https://ltecwithlance.gumroad.com/

Website: https://ltecwithlance.com/
